...
export default async function mountRouter(app) {
    logger.print("Mounting Router...");
    const // This is the main router. It bundles all sub-routes and mounts them to the application.
    apiPrefix = process.env.API_PREFIX || "/api/", apiVersion = process.env.API_VERSION || "0.0.0", apiPathPrefix = `${apiPrefix}${apiVersion}/`, routePaths = _getRoutePaths();
    logger.print("Generated Route Paths are: ", routePaths);
    tryMiddleware(await attachRoutes(app, routePaths, apiPathPrefix));
    return app;
}
async function attachRoutes(app, routePaths, apiPathPrefix) {
    for (const route of routePaths) {
        logger.print("Attaching Route...", { route });
        const API_RoutePath = `${apiPathPrefix}${route}`, module = await import(`./routes/${route}.routes.ts`);
